...
export default function Stats({ categories, entries }: Props) {
  ...
  const heatmapData = useMemo(() => {
    ...
    while (current <= now) {
      ...
      if (current.getDay() === 1 && current.getTime() !== monday.getTime()) {
        weekIndex++
      }
  ...
  const exportCSV = () => {
    ...
    link.href = url
    link.download = `技能时间记录_${new Date().toISOString().split('T')[0]}.csv`
    link.click()
  ...
  const exportPDF = () => {
    ...
    const categorySummary = topCategories.map(cat => {
      ...
      return `<tr>
        <td style="padding:8px;border-bottom:1px solid #eee"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${cat.color};margin-right:8px;vertical-align:middle"></span>${cat.name}</td>
        <td style="padding:8px;border-bottom:1px solid #eee;text-align:right">${formatMinutes(total)}</td>
        <td style="padding:8px;border-bottom:1px solid #eee;text-align:right">${(total / totalMin * 100).toFixed(1)}%</td>
      </tr>`
    }).join('')
    ...
    const entryRows = sorted.map(e => {
      ...
      return `<tr>
        ...
        <td style="padding:6px 8px;border-bottom:1px solid #f0f0f0">${path}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #f0f0f0">${e.description || '-'}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #f0f0f0;text-align:right">${e.duration}分钟</td>
      </tr>`
    }).join('')
...
<head>
  <meta charset="UTF-8">
  <title>技能时间记录报告</title>
  <style>
    body { font-family: -apple-system, "Microsoft YaHei", sans-serif; max-width: 800px; margin: 0 auto; padding: 40px 20px; color: #333; }
    ...
    th { text-align: left; padding: 8px; border-bottom: 2px solid #ddd; font-size: 12px; color: #888; }
    .footer { margin-top: 40px; padding-top: 16px; border-top: 1px solid #eee; font-size: 12px; color: #aaa; text-align: center; }
  </style>
</head>
<body>
  <h1>技能时间记录报告</h1>
  <div class="subtitle">生成于 ${new Date().toLocaleDateString('zh-CN')}</div>
  <div class="summary-cards">
    <div class="card"><div class="card-value">${formatMinutes(totalMin)}</div><div class="card-label">总投入时间</div></div>
    <div class="card"><div class="card-value">${entries.length}</div><div class="card-label">记录条数</div></div>
    <div class="card"><div class="card-value">${topCategories.length}</div><div class="card-label">技能分类</div></div>
  </div>
  <h2>分类汇总</h2>
        ...
        <>
          ...
          {pieData.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <h3 className="text-sm font-medium text-gray-700 mb-4">时间占比</h3>
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  {pieData.map(item => (
                    <div key={item.name} className="flex items-center gap-1.5 text-xs">
                      ...
                      <span className="text-gray-400">{formatMinutes(item.value)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* 柱状图：每周趋势 */}
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <h3 className="text-sm font-medium text-gray-700 mb-4">每周投入趋势</h3>
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barData}>
                  <XAxis dataKey="name" tick={{ fontSize: 12 }} />
                  <YAxis tick={{ fontSize: 12 }} tickFormatter={v => formatMinutes(Number(v))} />
                  <Tooltip formatter={(value) => formatMinutes(Number(value))} />
          ...
          {/* 热力图：每日活跃 */}
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <h3 className="text-sm font-medium text-gray-700 mb-4">每日活跃（近90天）</h3>
                    ...
                    {Array.from({ length: 7 }, (_, dayIdx) => {
                      ...
                      return (
                        <div
                          ...
                          style={{ backgroundColor: cell ? getHeatColor(cell.minutes) : '#f3f4f6' }}
